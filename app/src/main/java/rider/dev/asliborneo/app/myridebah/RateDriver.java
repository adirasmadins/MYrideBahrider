...
public class RateDriver extends AppCompatActivity {
    ...
    private void Submit_Rating_Detail(String key) {
       ...
       driverId = key;
        ...
        rating_detail_ref.child(driverId).push().setValue(rate).addOnCompleteListener(new OnCompleteListener<Void>() {
            @Override
            public void onComplete(@NonNull Task<Void> task) {
                rating_detail_ref.child(driverId).addValueEventListener(new ValueEventListener() {
                    @Override
                    public void onDataChange(DataSnapshot dataSnapshot) {
                        ...
                        Driverinformationref.child(driverId).updateChildren(driver_rating_update).addOnCompleteListener(new OnCompleteListener<Void>() {
                            @Override
                            public void onComplete(@NonNull Task<Void> task) {
                            ...
                            public void onFailure(@NonNull Exception e) {
                                waiting_dialog.dismiss();
                                Toast.makeText(RateDriver.this,e.getMessage(),Toast.LENGTH_LONG).show();
                            }
                        });
                    }
